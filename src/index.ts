...
import { allTools, annotationsFor, type ToolDef } from "./tools.js";
...
if (!token || !subdomain) {
  process.stderr.write(
    "Error: RELATION_API_TOKEN and RELATION_SUBDOMAIN environment variables are required.\n",
  );
  process.exit(1);
}
...
function getEnabledTools(): ToolDef[] {
  return allTools.filter((t) => {
    if (t.safety === "dangerous" && !enableDangerous) return false;
    if (t.safety === "master" && !enableMaster) return false;
    return true;
  });
}

function buildPathAndParams(
  tool: ToolDef,
...
const server = new Server(
  { name: "relation-mcp-server", version: "1.0.0" },
  { capabilities: { tools: {} } },
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    const { path, query, body } = buildPathAndParams(
      tool,
